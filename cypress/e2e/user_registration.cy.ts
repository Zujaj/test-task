describe('User Fills Up The Form & Gets Registered Successfully', () => {
  ...
  it('should submit the form when user enters valid information', () => {
    ...
    cy.isFormSubmissionSuccessful(true)

  })

  it('should prompt error toast when user leaves fillable fields empty', () => {

    let fillableFields: string[] = ['nameField', 'emailAddressField', 'passwordField', 'confirmPasswordField']

    for (let index = 0; index < fillableFields.length; index++) {
      const field = fillableFields[index];
      cy.getDataCy(field).type('   ')
      cy.isFormSubmissionSuccessful(false)
    }
  })
  
  it("should prompt error toast when user directly hits the 'Submit' button ", () => {
      cy.isFormSubmissionSuccessful(false)
  })
